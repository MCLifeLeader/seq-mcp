...
# Replace values below, or pass --seq-url and --seq-api-key when invoking this script.
...
if [[ "$USING_GENERIC_DOTENV" == "true" ]]; then
  if [[ -z "$SEQ_URL_ARG" || -z "$SEQ_API_KEY_ARG" ]]; then
    echo "This run is using a generated generic .env template at '$DOTENV_PATH'." >&2
    echo "Provide both --seq-url and --seq-api-key when using the generic template." >&2
    usage
    exit 1
  fi
fi
