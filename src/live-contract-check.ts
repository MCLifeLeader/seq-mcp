...
import { SEQ_ROUTE_CATALOG, type SeqRouteCatalogEntry } from "./route-catalog.js";
...
async function probeRoute(client: SeqClient, path: string): Promise<ProbeResult> {
  try {
...
async function main(): Promise<void> {
  ...
  const safeProbePaths = Array.from(
    new Set(
      SEQ_ROUTE_CATALOG.filter(isSafeProbeRoute).map((entry) => entry.path)
    )
  ).sort();
  ...
  process.exitCode =
    liveRoutesMissingFromCatalog.length > 0 || staleRoutes.length > 0 ? 1 : 0;
}
