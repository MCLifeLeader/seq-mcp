...
const configSchema = z.object({
    ...
    seqMaxRequestBytes: z
        .number()
        .int()
        .min(128)
        .max(1_048_576)
        .default(262_144),
    seqMaxResponseBytes: z
        .number()
        .int()
        .min(128)
        .max(8_388_608)
...
export function loadConfig(): ServerConfig {
    ...
    const maxRequestBytesRaw = parseOptionalInteger(
        process.env.SEQ_MAX_REQUEST_BYTES,
    );
    const maxResponseBytesRaw = parseOptionalInteger(
        process.env.SEQ_MAX_RESPONSE_BYTES,
    );
